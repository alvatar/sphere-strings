;; Copyright (C) 2008, 2011, 2012, 2013, 2014 Mikael More
...
(cond-expand
 (optimize
  (declare (standard-bindings) (extended-bindings) (block) (mostly-fixnum) (fixnum)))
 (debug
...
(define (utf8-u8vector->string u8v)
  (define (bad-input-error)
    (error "The input u8vector is not proper UTF-8."))
...
(define (string->utf8-u8vector s)
  (if (not (string? s)) (error "argument must be a string" s)) ; For type safety - argument should be a string.
  (substring->utf8-u8vector s 0 (string-length s)))

(define (substring->utf8-u8vector s start end)
  (if (not (string? s)) (error "argument 1 must be a string" s)) ; For type safety - argument should be a string.
  (if (not (and (fixnum? end) (fx<= 0 end (string-length s)))) (error "argument 3 must be a fixnum in the interval 0 to string-length of s" end))
  (if (not (and (fixnum? start) (fx<= 0 start end))) (error "argument 2 must be a fixnum in the interval 0 to end" start))
...
(define* (utf8-u8vector-port->string-reader port (buffer-size 1024))
  (define-macro (dbg #!rest a)
    ;; `(print "utf8-u8vector-port->string-reader: " ,@a "\n")
    '#!void)
